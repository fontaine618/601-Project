...
manova = MANOVA.from_formula("c0+c1+c2+c3+c4+c5+c6+c7+c8+c9~umpire+score*inning", data=df)
...
p_values = pd.DataFrame(
    {
        components_names[i]:
        sm.stats.anova_lm(ols("c" + str(i) + "~umpire+score*inning", data=df).fit(), typ=2)["PR(>F)"][1:4]
        for i in range(10)
    }
)
...
coefs = pd.DataFrame(
    {
        components_names[i]:
        ols("c" + str(i) + "~umpire+score*inning", data=df).fit().params[-5:]
        for i in range(10)
    }
)
...
plt.yticks(range(3), ["Score", "Inning", "Score-inning interaction"])
plt.title("Univariate ANOVAs: p-values", loc="left", fontweight="bold")
fig.tight_layout()
# plt.show()
plt.savefig("./fig/ols_score_inning_pvalues.pdf")

threshold = 0.05 / 3 # Bonferroni
fig = plt.figure(figsize=(8, 5))
coefs[p_values.to_numpy()[[0, 0, 1, 2, 2], :] > threshold] = np.nan
plt.imshow(coefs, cmap=newcmp, vmin=-0.2, vmax=0.2)
cbar = plt.colorbar()
plt.grid(False)
plt.xticks(range(10), p_values.columns, rotation=90)
plt.yticks(range(5), ["Batter lead", "Pitcher Lead", "First innings",
                      "Batter lead:First innings", "Pitcher Lead:First innings"])
plt.title("Univariate ANOVAs significant effects:\nestimated differences from close game and last innings",
          loc="left", fontweight="bold")
fig.tight_layout()
#plt.show()
plt.savefig("./fig/ols_score_inning_diffs.pdf")
